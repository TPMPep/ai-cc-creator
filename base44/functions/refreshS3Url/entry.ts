...
// AWS Signature V4 helpers (same as getS3UploadUrl)
...
async function generatePresignedGetUrl(bucket, region, key, accessKey, secretKey, expiresIn) {
...
Deno.serve(async (req) => {
  try {
    ...
    // Get storage profile for bucket/region
    ...
    const url = await generatePresignedGetUrl(
      profile.bucket, profile.region, s3Key, accessKey, secretKey,
      7 * 24 * 3600 // 7 days
    );
