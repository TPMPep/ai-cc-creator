...
export default function JobDetailAI() {
  ...
  // Call GPT directly from the frontend — no backend timeout constraints
  const callGPTBatch = async (batch, batchGaps, language, highlights, batchIndex, totalBatches) => {
    ...
    const gapInput = batchGaps.length > 0
      ? 'SILENCE GAPS:\n' + batchGaps.map(g => `${g.start}ms → ${g.end}ms (${Math.round((g.end - g.start)/1000)}s gap)`).join('\n')
      : '';
    const highlightDump = highlights?.length > 0 ? 'KEY AUDIO TERMS: ' + highlights.map(t => `"${t}"`).join(', ') : '';
    const batchNote = totalBatches > 1 ? `NOTE: Batch ${batchIndex + 1} of ${totalBatches}. Process only these segments.\n\n` : '';
...
3. Smart line breaks — keep context together
...
${highlightDump}`;
    ...
    throw new Error('OpenAI failed after 5 attempts');
  };
