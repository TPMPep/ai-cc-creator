...
${highlightDump}`;
  ...
  const hasSC = (t) => /\[[^\]]*\]/.test(t) || t.includes('♪');
  const isComplete = (t) => /[.?!]$/.test(t.replace(/\n/g, ' ').trim()) && t.split(/\s+/).length >= 2;
  const FW_RP = new Set(['a','an','the','of','to','and','or','but','with','from','in','on','at','for','that','is','are','was','were','by','as','it','its','my','our','your','his','her','their','this','not','be']);
  const endsFWrp = (ln) => { const w = ln.trim().split(/\s+/); return w.length > 0 && FW_RP.has(w[w.length - 1].replace(/[.,!?;:'"]+$/,'').toLowerCase()); };
  const repack = (text) => {
    if (hasSC(text)) return null;
...
function finalEnforce(cues) {
  ...
  for (const cue of cues) {
    ...
    // Single-speaker — smart repack
    const stripped = lines.map(l => l.replace(/^- /, '')).join(' ');
    const words = stripped.split(/\s+/).filter(Boolean);
    if (!words.length) continue;
    const FW_SS = new Set(['a','an','the','of','to','and','or','but','with','from','in','on','at','for','that','is','are','was','were','by','as']);
    const endsFWss = (ln) => { const w = ln.trim().split(/\s+/); return w.length > 0 && FW_SS.has(w[w.length - 1].replace(/[.,!?;:'"]+$/,'').toLowerCase()); };
    ...
    for (const word of words) {
      const cd = cur ? `${cur} ${word}` : word;
      if (cd.length <= MAX_CHARS) cur = cd;
