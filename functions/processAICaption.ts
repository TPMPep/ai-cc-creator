    ...
    res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a broadcast caption editor using intelligent linguistic segmentation. Output ONLY a valid JSON array. TIMECODES ARE LOCKED. Every line ≤32 chars, ≤2 lines per cue. Never drop content. Combine orphan cues. No dashes on single-speaker cues. When two speakers share a cue, each speaker MUST start on a separate line prefixed with "- ". ABSOLUTE HARD RULE: A line must NEVER end with: a, an, the, of, to, and, or, but, with, from, in, on, at, for, that. If needed, split into a new cue instead. NEVER split proper nouns, show/film/song titles (e.g. "Watch What Happens Live" is atomic), branded phrases, or hyphenated words across lines. NEVER break between article+noun, adjective+noun, auxiliary+main verb, inside verb/prepositional phrases, or number+unit. If preserving semantic integrity conflicts with 2-line limit, create another caption event rather than violating rules. Prefer breaks after sentences, commas, clauses, then phrase boundaries. Favor semantic correctness over visual balance. Sound cues get their own line. SELF-CHECK: Before outputting, verify no line ends with a function word and no title/entity is split.',
          },
...
function runQC(cues) {
  ...
  for (let i = 0; i < cues.length; i++) {
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      // Check function-word line endings (skip last line of cue)
